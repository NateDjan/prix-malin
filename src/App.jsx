...
const STORES = [
  ...
  { id: 'intermarche', name: 'Intermarche',  letter: 'I', color: '#E31E24', factor: 1.04 },
...
const DRIVE = {
  leclerc:     { note: 'Connecte-toi sur leclercdrive.fr avant de demarrer', url: q => `https://fd3-courses.leclercdrive.fr/magasin-169203-169203-Rueil-Malmaison-Boulevard-National/recherche.aspx?TexteRecherche=${encodeURIComponent(q)}` },
  carrefour:   { note: 'Connecte-toi sur carrefour.fr avant de demarrer',   url: q => `https://www.carrefour.fr/s?q=${encodeURIComponent(q)}&ref=search` },
  intermarche: { note: 'Connecte-toi sur intermarche.com avant de demarrer', url: q => `https://www.intermarche.com/recherche?q=${encodeURIComponent(q)}` },
  auchan:      { note: 'Connecte-toi sur auchan.fr avant de demarrer',      url: q => `https://www.auchan.fr/recherche?q=${encodeURIComponent(q)}` },
  monoprix:    { note: 'Connecte-toi sur monoprix.fr avant de demarrer',    url: q => `https://www.monoprix.fr/recherche/${encodeURIComponent(q)}` },
...
async function fetchRealPrices(products, cp) {
  ...
  await Promise.all(top.map(async p => {
    try {
      const d = await fetch(`/api/prix?q=${encodeURIComponent(p.search)}&cp=${encodeURIComponent(cp)}`).then(r => r.json())
      if (d.prices) prices[p.search] = d.prices
    } catch {}
  }))
  return prices
}
...
function getRealPrice(search, storeId, realPrices) {
  const key = Object.keys(realPrices).find(k =>
    k === search || k.toLowerCase() === search.toLowerCase() ||
    search.toLowerCase().startsWith(k.toLowerCase().split(' ').slice(0,2).join(' ')) ||
    k.toLowerCase().startsWith(search.toLowerCase().split(' ').slice(0,2).join(' '))
  )
  if (!key) return null
  ...
  return (
    <div className="prog-bar">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: done ? 0 : 8 }}>
        <span style={{ color:'#5BF5A8', fontWeight:800, fontSize:13 }}>
          {done ? `✅ ${products.length} produits sur ${storeName}` : `🛒 ${cur}/${products.length} · ${storeName}`}
        </span>
        <button onClick={onClose} style={{ background:'none', border:'1px solid rgba(240,237,232,.2)', color:'rgba(240,237,232,.4)', padding:'4px 10px', borderRadius:8, cursor:'pointer', fontSize:11, fontFamily:'inherit' }}>
          {done ? 'Fermer' : 'Annuler'}
        </button>
      </div>
      {!done && <>
        <div style={{ background:'rgba(255,255,255,.08)', borderRadius:4, height:6, marginBottom:6 }}>
          <div style={{ background:'#5BF5A8', height:6, borderRadius:4, width:`${pct}%`, transition:'width .4s' }} />
        </div>
        <div style={{ fontSize:12, color:'rgba(240,237,232,.5)' }}>📦 {products[cur]?.search}</div>
      </>}
    </div>
  )
}
...
function CompareView({ result, realPrices, cp, onSetCp, onFetchPrices }) {
  ...
  return (<>
    <div className="ticket-info">
      <div>
        <div className="ticket-store">{storeName || 'Ticket'}</div>
        <div className="ticket-meta">{date ? `Le ${date} · ` : ''}{products.length} articles</div>
      </div>
      <div className="ticket-total">{(total||base).toFixed(2)} €</div>
    </div>

    <div className="sec-title">CHOISIR UNE ENSEIGNE</div>
    {hasReal && <div className="real-badge">✓ Prix reels disponibles</div>}

    <div className={`ecomix ${store === 'ecomix' ? 'sel' : ''}`} onClick={() => setStore('ecomix')}>
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        <div style={{ fontSize:28 }}>🌿</div>
        <div><div className="em-name">Eco-Mix</div><div className="em-sub">Chaque produit chez le moins cher</div></div>
      </div>
      <div style={{ textAlign:'right' }}>
        <div className="em-amt">{ecoAmt} €</div>
        <div className="em-save">-{(base - parseFloat(ecoAmt)).toFixed(2)} € eco.</div>
      </div>
    </div>

    <div className="stores-grid">
      {STORES.map(s => {
        const ct = calcStoreTotal(products, s.id, realPrices)
        return (
          <div key={s.id} className={`store-card ${store === s.id ? 'sel' : ''}`} onClick={() => setStore(s.id)}>
            ...
            <div className={`store-lbl ${ct.realCount > 0 ? 'real' : ''}`}>
              {ct.realCount > 0 ? `${ct.realCount}/${ct.totalCount} reels` : 'estimation'}
            </div>
          </div>
        )
      })}
